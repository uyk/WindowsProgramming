...
void CBrightnessContrastDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_BRIGHTNESS_CSLIDER, m_sliderBrightness);
	...
	DDX_Text(pDX, IDC_CONTRAST_EDIT2, m_nContrast);
...
BEGIN_MESSAGE_MAP(CBrightnessContrastDlg, CDialogEx)
	...
	ON_EN_CHANGE(IDC_CONTRAST_EDIT2, &CBrightnessContrastDlg::OnEnChangeContrastEdit2)
END_MESSAGE_MAP()
...
void CBrightnessContrastDlg::OnEnChangeContrastEdit2()
{
	// TODO:  RICHEDIT 컨트롤인 경우, 이 컨트롤은
	// CDialogEx::OnInitDialog() 함수를 재지정 
	//하고 마스크에 OR 연산하여 설정된 ENM_CHANGE 플래그를 지정하여 CRichEditCtrl().SetEventMask()를 호출하지 않으면
